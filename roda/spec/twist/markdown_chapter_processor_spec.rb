...
describe Twist::MarkdownChapterProcessor do
  let(:book_repo) { Twist::BookRepository.new }
  let(:branch_repo) { Twist::BranchRepository.new }
  let(:element_repo) { Twist::ElementRepository.new }
  let(:image_repo) { Twist::ImageRepository.new }
  ...
  subject do
    Twist::MarkdownChapterProcessor.new(
      commit,
      git.local_path,
