...
module Twist
  describe "Remove Reader", type: :request do
    ...
    before do
      invite.(inviter: radar, book_id: book.id, user_id: ghost.id)
    end
    ...
    context "when the remover is not an author" do
      it "does not remove the user" do
        ...
        remove_reader = json_body.dig("data", "removeReader")
        expect(remove_reader["error"]).to eq("You must be an author to do that.")
      end
    end
    ...
    context "when the inviter is an author" do
      ...
      it "removes the reader" do
        ...
        expect(remove_reader["bookId"]).to eq(book.id.to_s)
        expect(remove_reader["userId"]).to eq(ghost.id.to_s)
